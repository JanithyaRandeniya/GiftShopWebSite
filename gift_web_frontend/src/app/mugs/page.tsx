...
const mugs = [
  { id:301, name:"Romantic Mug Duo",          price:2800, badge:"Popular", img:"/images/gifts/lovemug.webp",     hoverImg:"/images/gifts/photoframe1.webp", emoji:"☕", desc:"Matching 'You & Me' porcelain mugs for couples",     material:"Porcelain",     capacity:"350ml", rating:4, reviews:62  },
  { id:302, name:"Floral Tea Gift Set",        price:3300, badge:"New",     img:"/images/gifts/photoframe1.webp", hoverImg:"/images/gifts/lovemug.webp",     emoji:"🌸", desc:"4 artisan teas + 2 floral hand-painted cups",         material:"Ceramic",       capacity:"280ml", rating:4, reviews:41  },
  { id:303, name:"Personalised Photo Mug",     price:1800, badge:"Custom",  img:"/images/gifts/lovemug.webp",     hoverImg:"/images/gifts/boxwithflower1.jpg",emoji:"📸", desc:"Upload your photo — printed with archival ink",         material:"Ceramic",       capacity:"300ml", rating:4, reviews:167 },
  { id:304, name:"Gold-Rim Luxury Mug Set",    price:4200, badge:"Premium", img:"/images/gifts/photoframe1.webp", hoverImg:"/images/gifts/wallet1.webp",     emoji:"✨", desc:"Set of 4 ceramic mugs with 22k gold rim",               material:"Fine Ceramic",  capacity:"320ml", rating:5, reviews:38  },
  { id:305, name:"Travel Mug with Engraving",  price:2600, badge:"",        img:"/images/gifts/lovemug.webp",     hoverImg:"/images/gifts/mensgift1.webp",   emoji:"🚀", desc:"Double-wall stainless steel, custom engraved",          material:"Stainless Steel",capacity:"450ml",rating:5, reviews:55  },
  { id:306, name:"His & Hers Matching Set",    price:3600, badge:"Popular", img:"/images/gifts/photoframe1.webp", hoverImg:"/images/gifts/lovemug.webp",     emoji:"💑", desc:"Two mugs, two hearts — perfect couple gift",            material:"Porcelain",     capacity:"380ml", rating:5, reviews:91  },
];
...
const styles = `
  ...
  .mugs-hero { background:linear-gradient(135deg,#e8f4f8 0%,#f0f4ff 50%,#fde8ef 100%); padding:72px 48px 56px; position:relative; overflow:hidden; }
  .mugs-hero::after { content:'☕'; position:absolute; right:60px; top:50%; transform:translateY(-50%); font-size:200px; opacity:0.06; pointer-events:none; }
  ...
  .mugs-hero h1 { font-family:'Libre Baskerville',serif; font-size:clamp(2.2rem,4.5vw,3.5rem); color:#2a3060; margin-bottom:12px; line-height:1.15; }
  ...
  .mugs-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:28px; }
  .mc-card { background:white; border-radius:22px; overflow:hidden; box-shadow:0 4px 20px rgba(42,48,96,0.08); border:1px solid rgba(90,127,212,0.1); transition:transform 0.3s,box-shadow 0.3s; position:relative; }
  .mc-card:hover { transform:translateY(-6px); box-shadow:0 16px 40px rgba(90,127,212,0.18); }
  ...
  .mc-img-overlay { position:absolute; left:0; right:0; bottom:0; height:0; overflow:hidden; transition:height 0.45s cubic-bezier(0.4,0,0.2,1); }
  .mc-card:hover .mc-img-overlay { height:60%; }
  .mc-img-overlay-inner { position:absolute; bottom:0; left:0; right:0; height:230px; }
  .mc-badge { position:absolute; top:14px; left:14px; z-index:3; padding:5px 14px; border-radius:50px; font-size:0.68rem; font-weight:700; letter-spacing:0.5px; color:white; text-transform:uppercase; background:linear-gradient(135deg,#5a7fd4,#2a3060); }
  ...
  .mc-spec { background:rgba(90,127,212,0.08); color:#2a3060; font-size:0.7rem; font-weight:600; padding:3px 10px; border-radius:50px; }
  ...
  .mc-cart { background:linear-gradient(135deg,#2a3060,#5a7fd4); color:white; border:none; padding:9px 18px; border-radius:50px; font-family:'Outfit',sans-serif; font-weight:600; font-size:0.8rem; cursor:pointer; transition:all 0.2s; }
  .mc-cart:hover { transform:scale(1.05); box-shadow:0 4px 14px rgba(90,127,212,0.35); }
  .custom-mug-banner { background:linear-gradient(135deg,rgba(90,127,212,0.06),rgba(232,84,122,0.06)); border-radius:24px; padding:52px 48px; display:flex; gap:48px; align-items:center; flex-wrap:wrap; margin-top:64px; border:2px dashed rgba(90,127,212,0.25); }
  ...
  .g-toast { position:fixed; bottom:32px; left:50%; transform:translateX(-50%); background:linear-gradient(135deg,#2a3060,#5a7fd4); color:white; padding:14px 28px; border-radius:50px; font-family:'Outfit',sans-serif; font-weight:600; font-size:0.9rem; box-shadow:0 8px 24px rgba(42,48,96,0.3); z-index:9999; animation:toastIn 0.3s ease; }
  @keyframes toastIn { from{opacity:0;transform:translateX(-50%) translateY(10px)}to{opacity:1;transform:translateX(-50%) translateY(0)} }
  @media(max-width:768px){ .mugs-hero,.g-section{padding:48px 20px} .custom-mug-banner{padding:36px 24px;flex-direction:column} }
`;
...
export default function MugsPage() {
  ...
  const handleAdd = (m: typeof mugs[0]) => {
    addToCart({ productId: m.id, name: m.name, price: m.price, image: m.img, emoji: m.emoji });
            ...
            {mugs.map(m => (
              <div key={m.id} className="mc-card">
                <div className="mc-img-wrap">
                  {m.badge && (
                    <span className={`mc-badge${m.badge==="Custom"?" custom":m.badge==="Premium"?" premium":""}`}>{m.badge}</span>
                  )}
                ...
                <div className="mc-info">
                  ...
                  <div className="mc-footer">
                    ...
                    <button className="mc-cart" onClick={() => handleAdd(m)}>Add to Cart</button>
                  </div>
                </div>
              </div>
            ))}
